...
$(document).ready(function() {
	...
	if($('[name="periode_type"]').val() == 1 )
	{
		...
		$('[name="filter"]').chosen().chosenReadonly(true);
		$('[name="filter"]').val('1').trigger('chosen:updated');
	}
	else
	{
		...
		$('[name="filter"]').chosen().chosenReadonly(false);
		$('[name="filter"]').val('<?php echo $xfilter; ?>').trigger('chosen:updated');
	}
